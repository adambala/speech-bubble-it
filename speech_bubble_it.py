...
def check_file_format(file_path: Path, supported_formats: tuple) -> None:
    ...
    if file_path.suffix not in supported_formats:
        raise ValueError(f"Unsupported file extension {file_path.suffix}. Only {supported_formats} are supported.")
...
def process(image_path: Path, output_path: Path, mirror: bool, orientation: int) -> None:
    ...
    original_image = open_image(image_path).convert("RGBA") # RGBA to ensure transperency in the result
    speech_bubble_image = open_image(Path(os.path.join(ASSETS_DIR, SPEECH_BUBBLE_FILENAME)))

    ImageOps.exif_transpose(original_image, in_place=True) # to prevent unexpected rotation of images with EXIF orientation tag != 1
    speech_bubble_image = speech_bubble_image.resize(original_image.size)
    speech_bubble_image = transform_image(speech_bubble_image, mirror, orientation)
...
def main() -> None:
    ...
    parser.add_argument("image_path", type=Path, metavar="image_file", help="path of an image you want to put speech bubble on")
    parser.add_argument("output_path", type=Path, metavar="output_file", help="path to the file where the result will be written to")
    parser.add_argument("-m", "--mirror", action=BooleanOptionalAction, help="flips speech bubble horizontally")
    parser.add_argument("-o", "--orientation", type=int, default=1, choices=(1, 2, 3, 4), help="changes speech bubble orientation (1: on the top; 2: on the left; 3: on the bottom; 4: on the right)")
